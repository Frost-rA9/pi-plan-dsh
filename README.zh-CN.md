...
- 激活期间，每次 agent 启动都注入一段 `plan:policy` 提示——只改变提示，从不改变工具目录。
- `exit_plan_mode` 无论 plan 模式是否激活都保持注册，因此进入/退出只改变提示段；以 `#` 标题开头的完整 markdown 计划呈现给用户审阅；批准即退出 plan 模式，「keep planning」把反馈发回模型。
...
4. 批准交互从不进入模型上下文；结果经工具结果/一次失败调用返回。
5. **失败即关闭**：无交互通道、审阅期间重载、或已驳回的审阅 → `exit_plan_mode` 失败，`/plan off` 作为逃生出口。
6. 与 `pi-sandbox-dsh` 完全正交——无共享状态或类型。

## 后端

无——这是纯引导扩展（无 OS 沙箱、无 preview/question 能力 seam）。设计依据与已知取舍见本地 `AGENTS.md`（gitignored，不入库）。
